<?php
	...
	<table class="sub_table">
		...
		<tr>
			<th class="sub_td_button" style="text-align: right;"><input type="button" class="board-button" value="Write" onClick="javascript:goWrite(1);"></th>
		</tr>
		<tr>
			<th class="sub_td">
	';
	...
	while( $row = $result->fetch_assoc() ) {
		$content .= '
					<tr>
						...
						<td onClick="javascript:goBoradDetail(\'BB_010W.php?no='.$row["BB_NO"].'\', 1, '.$nextPage.');" style="cursor:pointer">'.$row["BB_TITLE"].'</td>
						<td align="center">';
		if( $row["ATTCH_YN"] == 'Y' ) {
			$content .= '<a href="board/'.$row["BB_ATTACH_FILE"].'" download title="'.$row["BB_ATTACH_FILE"].'"><i class="fa fa-file-image-o" aria-hidden="true"></i></a>';
		}
		$content .= '
						</td>
